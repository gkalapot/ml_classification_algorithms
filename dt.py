    ...
    def train_dt(self, X, y, remaining_depth, used_features):
        ...
        if remaining_depth <= 0 or len(np.unique(y)) <= 1:
            ...
            for i in range(d):
                ...
                feature_values = X[:, i]
                left_mask = feature_values < 0.5
                right_mask = feature_values >= 0.5
                ...
                left_positives = np.sum(y_left == 1.0)
                left_negatives = len(y_left) - left_positives
                left_score = max(left_positives, left_negatives)

                right_positives = np.sum(y_right == 1.0)
                right_negatives = len(y_right) - right_positives
                right_score = max(right_positives, right_negatives)
            ...
            if best_feature is None:
                ...
                new_used_features.append(best_feature)

                self.left.train_dt(X_left, y_left, remaining_depth - 1, new_used_features)
                self.right.train_dt(X_right, y_right, remaining_depth - 1, new_used_features)
